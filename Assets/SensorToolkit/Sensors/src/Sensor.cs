...
namespace Micosmo.SensorToolkit {
     ...
     *  - Either: The sensor detects the RigidBody if there is one, otherwise it will detect the collider.
     */
    public enum DetectionModes { Colliders, RigidBodies, Either }
    ...
    public abstract class Sensor : BasePulsableSensor {
         ...
         */
        public List<SignalProcessor> SignalProcessors => signalProcessors;
        [SerializeField] List<SignalProcessor> signalProcessors = new List<SignalProcessor>();
        ...
        public override void Clear() {
            signalPipeline.UpdateAllInputs(clearList);
        }
        ...
        public List<Signal> GetSignalsBySignalStrength(Predicate<Signal> predicate, List<Signal> storeIn = null) => OrderedBySignalStrength(GetSignals(predicate, storeIn));
        ...
        protected SignalFilter SignalFilter;
        protected MapToRigidBodyFilter<GameObject, Signal> MapToRigidBody;
        protected MapToSignalProxyFilter<GameObject, Signal> MapToSignalProxy;

        // Must be non serialized or we get some data loss on domain reload
        [NonSerialized] SignalPipeline _signalPipeline;
        protected SignalPipeline signalPipeline {
            get {
                if (_signalPipeline == null) {
                    ...
                    _signalPipeline.OnChange += delegate (Signal prev, Signal next) {
                        OnSignalChanged?.Invoke(next, this);
                    };
                    _signalPipeline.OnRemove += delegate (Signal signal) {
                        OnSignalLost?.Invoke(signal, this);
                        OnLostDetection.Invoke(signal.Object, this);
                    };
                    _signalPipeline.OnSome += delegate {
                        OnSomeDetection.Invoke();
                    };
                    _signalPipeline.OnNone += delegate {
                        OnNoDetection.Invoke();
                    };
                    _signalPipeline.SignalProcessorCallback = new AccumulatorPipeline<GameObject, Signal>.SignalProcessor(ProcessSignal);
        ...
        protected void LostSignalImmediate(GameObject forObject) {
            signalPipeline.RemoveInput(forObject);
        }
        ...
        bool ProcessSignal(in Signal input, out Signal processed) {
            var processedSignal = input;
            if (!MapToRigidBody.ProcessOutput(ref processedSignal)) {
                processed = default;
                return false;
            }
            if (!MapToSignalProxy.ProcessOutput(ref processedSignal)) {
                processed = default;
                return false;
            }
            if (signalProcessors != null) {
                foreach (var processor in signalProcessors) {
                    ...
                    if (!processor.Process(ref processedSignal, this) || ReferenceEquals(processedSignal.Object, null)) {
                        processed = default;
                        return false;
                    }
                }
